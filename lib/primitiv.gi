...
##
##  The quotient PGammaL(dim,q)/PSL(dim,q) is generated by the images of the
##  diagonal matrix delta = diag(Z(q),1,...,1) and of the Frobenius phi.  The
##  image of delta has order g = gcd(dim,q-1), so delta^g lies in PSL and only
##  i mod g matters; that of phi has order f, where q = p^f.  The pair [i,j]
##  is the coset of delta^i phi^j, and first delta^i1 phi^j1 then
##  delta^i2 phi^j2 is delta^(i1 + i2*p^(f-j1)) phi^(j1+j2).
##
##  Return the pairs of the subgroup <auts> generates in that quotient.  How
##  many there are is how many cosets of PSL(dim,q) the group they describe
##  has, so that group's order is their number times the order of PSL(dim,q).
##
