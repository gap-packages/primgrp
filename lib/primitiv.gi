...
##
##  Modulo PSL(dim,q), the group PGammaL(dim,q) is generated by the diagonal
##  automorphism delta = diag(Z(q),1,...,1), of order g = gcd(dim,q-1), and
##  the Frobenius phi, of order f, where q = p^f.  The pair [i,j] stands for
##  delta^i phi^j, and first delta^i1 phi^j1 then delta^i2 phi^j2 is
##  delta^(i1 + i2*p^(f-j1)) phi^(j1+j2).
##
##  Return the subgroup <auts> generates under that product, as the set of its
##  pairs.  How many there are is the index over PSL, and so a factor of the
##  order of any group described this way.
##
...
##  pair [i,j].  That reaches every group between PSL and PGammaL, including
##  those the four named ones miss, such as
##  PSL(2,25).2_3 = PSL(2,25).<delta phi>.  The points are the sorted normed
##  vectors.
##
