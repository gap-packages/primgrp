...
##  nothing.
##
##  The decomposition itself is tst/product4c.g, which is read below.  Nothing
##  rests on its socle components being the right ones: a wrong factor gives
...
PRIMGRP_Convert4c := function(e, deg)
  ...
  # A field 9 that names a construction, the "4c" this pass writes among them,
  # is left alone; anything else is the generators themselves.
  if Length(e[9]) > 0 and IsStringRep(e[9][1]) then
    return fail;
  fi;
  gens := e[9];
